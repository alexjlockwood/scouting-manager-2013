...
public class ScoutProvider extends ContentProvider {
	...
	static {
		...
		sUriMatcher.addURI(ScoutContract.AUTHORITY, "team_matches",
				TEAM_MATCHES);
		sUriMatcher.addURI(ScoutContract.AUTHORITY, "team_matches/team/#",
				TEAM_MATCHES_TID);
		sUriMatcher.addURI(ScoutContract.AUTHORITY,
				"team_matches/match/#/team/#", TEAM_MATCHES_MID_TID);
	}
	...
	public Cursor query(Uri uri, String[] projection, String where,
		...
		if (DEBUG) {
			Log.v(TAG,
					"query(uri=" + uri + ", proj="
							+ Arrays.toString(projection) + ", where=" + where
		...
		case TEAM_MATCHES_TID:
			qb.setTables(Tables.TEAM_MATCHES);
			qb.appendWhere(TeamMatches.TEAM_ID + " = "
					+ uri.getLastPathSegment());
			break;
		// Get a cursor over a team's specific match in the team_matches table.
		case TEAM_MATCHES_MID_TID:
			qb.setTables(Tables.TEAM_MATCHES);
			qb.appendWhere(TeamMatches.MATCH_ID + " = "
					+ uri.getPathSegments().get(2) + " AND "
		...
		Cursor cur = qb.query(mOpenHelper.getReadableDatabase(), projection,
